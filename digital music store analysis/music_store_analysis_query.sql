...
with customer_and_country as(
	select customer.customer_id, customer.first_name || ' ' || customer.last_name as full_name, customer.country, 
		sum(invoice.total) as total_spending,
		row_number() over(partition by customer.country order by sum(invoice.total) desc) as row_num
	from customer
	join invoice on invoice.customer_id = customer.customer_id
	group by 1, 3
